...
(2).@Adaptive方法上，动态生成适配器类 目的是为了通过URL/Invoker来匹配到对应的实现类
```java
@SPI("javassist")
public interface ProxyFactory {

    // PROXY_KEY = proxy 如果URL中配置了proxy则使用动态适配器扩展点，否则使用默认扩展点javassist
    @Adaptive({PROXY_KEY})
...
条件激活，Dubbo中主要用在Filter里面
类似于Spring中的@Conditional
```java
