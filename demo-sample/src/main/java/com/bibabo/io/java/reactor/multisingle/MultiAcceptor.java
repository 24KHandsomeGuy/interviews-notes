...
import com.bibabo.io.java.reactor.singlesingle.ReadHandler;
...
public class MultiAcceptor implements Runnable {
    ...
    public void run() {
        try {
            ...
            // 读处理
            socketChannel.register(selector, SelectionKey.OP_READ, new MultiReadHandler(selector, socketChannel));
        } catch (Exception e) {
            e.printStackTrace();
        }
